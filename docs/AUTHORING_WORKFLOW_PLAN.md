...
1. ~~**Skeletal animation.**~~ ✅ **LANDED.** Model → rig → animate in Blender → export `.glb` →
   it loads, deforms and plays. `Animation/Skin.cs` (joint palette), `AnimationClip.cs` (samplers,
   channels, blending), `Animator.cs` (play / crossfade / seek / speed),
   `SkinnedMeshComponent.cs`, `SkinnedMeshShader.cs` (linear blend skinning, sharing the static
   shaders' fragment stages so lighting can't drift), a skinned lane in `MeshRenderer`, and
   `GlbLoader` support for `skins` + `animations`. Joints are ordinary `SimObject`s, so parenting a
   sword to a hand or reading a bone's world matrix needs no new machinery.

   Verified by `sketches/skin_test.cs` against a rigged capsule generated by
   `tools/make_test_rig.py` (no Blender needed): the skin and both clips parse, the clip rotates
   the joint by exactly the authored 60°, the rest palette is a no-op on the bind pose, the
   weighted tip moves 1.04 units while the root-weighted base moves 0.0000, playback advances on
   its own, and a mid-crossfade pose sits between both clips rather than snapping to either.

   Scope held: max 64 joints and 4 influences per vertex (both enforced with a clear error, not
